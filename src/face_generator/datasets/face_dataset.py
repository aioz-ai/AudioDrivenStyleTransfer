    ...
    def draw_shoulder_points(self, img, shoulder_points):
        ...
        for i in range(2):
            for j in range(num - 1):
                img = cv2.line(img, tuple(shoulder_points[i * num + j].astype(np.int)), tuple(shoulder_points[i * num + j + 1].astype(np.int)), 255, 2)  # BGR
        ...
        for edge_list in self.part_list:
            for edge in edge_list:
                for i in range(len(edge)-1):
                    im_edges = cv2.line(im_edges, tuple(keypoints[edge[i]].astype(np.int)), tuple(keypoints[edge[i+1]].astype(np.int)), 255, 2)
