...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1):
    ...
    return f'ffmpeg -y -i {inp} -ss {start} -t {time} -filter:v "crop={w}:{h}:{left}:{top}, scale={scale}" crop.mp4'


def compute_bbox_trajectories(trajectories, fps, frame_shape, min_frames, inp, image_shape, increase):
    commands = []
...
def process_video(input, image_shape=(256,256), increase=0.1, iou_with_initial=0.25, min_frames=0, cpu=False):
    device = 'cpu' if cpu else 'cuda'
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False, device=device)
    video = imageio.get_reader(input)
    ...
    try:
        for i, frame in tqdm(enumerate(video)):
            ...
            commands += compute_bbox_trajectories(not_valid_trajectories, fps, frame_shape, min_frames, input, image_shape, increase)
    ...
    commands += compute_bbox_trajectories(trajectories, fps, frame_shape, min_frames, input, image_shape, increase)
    return commands


def crop_video_with_cmd(command, output_filepath):
    _ = os.system(command[0].replace('crop.mp4', output_filepath))
