...
def chunks(lst, n):
    for i in range(0, len(lst), n):
        yield lst[i:i + n]
def create_batch_kp(kp_list, batchsize):
    new_list = [] 
    val_only = [x['value'] for x in kp_list]
    padnum = 0 
    if len(val_only) % batchsize != 0: 
        nn = len(val_only)
        padnum = batchsize - (nn%batchsize)
        for i in range(padnum):
            val_only.append(val_only[-1])
    assert len(val_only)% batchsize == 0 
    kp_chunks = chunks(val_only, batchsize)
    for it in kp_chunks:
        
        new_list.append({'value': torch.cat(it,0), 'jacobian':None})

    return new_list, padnum 

def make_animation(source_image, driving_video, generator, kp_detector, he_estimator, relative=True, adapt_movement_scale=True, estimate_jacobian=True, cpu=False, free_view=False, yaw=0, pitch=0, roll=0, batchsize = 1):
    with torch.no_grad():
        ...
        print(summary(kp_detector,input_data=source, col_names=["kernel_size", "output_size", "input_size", "num_params"], depth = 10))
        print(summary(he_estimator,input_data=source, col_names=["kernel_size", "output_size", "input_size", "num_params"],depth = 10))
        ...
        source = source.repeat(batchsize,1,1,1)
        kp_norm_list = []
        for frame_idx in tqdm(range(0,driving.shape[2])):
            # gc.collect()
        ...
        kp_norm_batch, padnum = create_batch_kp(kp_norm_list, batchsize) 

        kp_source['value'] = kp_source['value'].repeat(batchsize,1,1)
        
        for i,kp_norm in enumerate(tqdm(kp_norm_batch, total=len(kp_norm_batch))): 
            print(kp_norm)    
            
            print(summary(generator,input_data=[source, kp_norm['value'], kp_source['value'] ], col_names=["kernel_size", "output_size", "input_size", "num_params"], depth = 10))
            # out = generator(source, kp_source=kp_source, kp_driving=kp_norm)
            break
            if i < len(kp_norm_batch) - 1:
                for pred in out['prediction'].data:
                    predictions.append(np.transpose(pred.unsqueeze(0).cpu().numpy(), [0, 2, 3, 1])[0])
            else:
                for pred in out['prediction'][:batchsize-padnum].data:
                    predictions.append(np.transpose(pred.unsqueeze(0).cpu().numpy(), [0, 2, 3, 1])[0])
    return predictions
...
if __name__ == "__main__":
    ...
    parser.add_argument("--result_video", default='./output.mp4', help="path to output")
    ...
    print("total generator param: ", count_parameters(generator))
    ...
    add_audio(opt.result_video, opt.driving_video)
